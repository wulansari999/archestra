...
describe("MCP Server Install - Preset Values reach Deployment Env", () => {
  ...
  test("plain_text preset env supplied via request-time presetFieldValues reaches startServer environmentValues", async ({
    makeInternalMcpCatalog,
  }) => {
    // The "Preset fields not filled" section of the install dialog sends
    // values via `presetFieldValues` on the install request. The route must
    // both persist them onto the catalog row AND surface them in the env
    // overlay used for this install's deployment — otherwise the pod ships
    // without the env var even though the catalog row stores it.
    const catalog = await makeInternalMcpCatalog({
      name: "preset-env-deploy-request-plain",
      serverType: "local",
      localConfig: {
        command: "node",
        arguments: ["server.js"],
  ...
  test("secret preset env supplied via request-time presetFieldValues reaches startServer environmentValues", async ({
    makeInternalMcpCatalog,
  }) => {
    // Secret preset env vars supplied at install time are partitioned into
    // the catalog row's preset secret bag. They must also surface into
    // environmentValues so the env builder emits a secretKeyRef for the
    // pod — without that, the K8s Secret has the data but the pod gets no
    // env var.
    const catalog = await makeInternalMcpCatalog({
      name: "preset-env-deploy-request-secret",
      serverType: "local",
      localConfig: {
        command: "node",
        arguments: ["server.js"],
    ...
    const response = await app.inject({
      ...
      payload: {
        name: "preset-env-deploy-request-secret",
        catalogId: catalog.id,
        presetFieldValues: { INTERNAL_TOKEN: "request-secret-val" },
      },
    });
    ...
    expect(k8sStartServerMock).toHaveBeenCalledTimes(1);
    const [, , envValues] = k8sStartServerMock.mock.calls[0];
    expect(envValues).toMatchObject({ INTERNAL_TOKEN: "request-secret-val" });
  });
});
