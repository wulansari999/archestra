...
-- drizzle-migration-linter: reason=New table for this feature; the partial unique index enforces one org-wide (NULL-environment) default per org and the data migration below inserts at most one such row per org, so no duplicates can pre-exist. FKs are added NOT VALID to avoid a validation lock on the large interactions table.
