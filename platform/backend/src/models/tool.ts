...
class ToolModel {
  ...
  static async seedArchestraTools(
    ...
    const discoveredArchestraTools = discoveredTools.filter((tool) => {
      ...
      // Adopt current built-ins (shortName resolves) and legacy platform tools
      // (folded into archestra__api) so the stale-delete path below can migrate
      // their assignments and remove them — a pre-catalog org may still hold
      // these legacy tools with catalog_id = NULL.
      if (shortName !== null) {
        return true;
      }
      const { toolName: rawShortName } = parseFullToolName(tool.name);
      return LEGACY_PLATFORM_TOOL_SHORT_NAMES.has(rawShortName);
    });
